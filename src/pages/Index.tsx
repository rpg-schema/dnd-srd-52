...
const Index = () => {
  ...
  const description = data.ontologyMeta?.comment || data.ontologyMeta?.properties?.['description']?.[0] || 'D&D SRD 5.2 profile for RPG Schema';
  ...
  return (
    <div className="min-h-screen bg-background">
      <OntologyHero title="D&D SRD 5.2" description={description} />
      <StickyNav sections={NAV_SECTIONS} ttlUrl={TTL_URL} />

      <OntologySection id="classes" index={idx()} title="Classes" subtitle="OWL class definitions in the SRD 5.2 ontology">
      ...
      <OntologySection id="properties" index={idx()} title="Properties" subtitle="Object and datatype properties linking SRD concepts">
      ...
      <OntologySection id="abilities" index={idx()} title="Ability Scores" subtitle="The six core abilities and their canonical mappings">
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {abilityScores.map((e) => (
      ...
      <OntologySection id="actions" index={idx()} title="Actions" subtitle="Action economy entries available in combat and exploration">
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {actions.map((e) => (
            <EntityCard key={e.uri} entity={e} />
          ))}
